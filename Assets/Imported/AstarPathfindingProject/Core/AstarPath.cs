...
#region Callbacks
	...
	*/
	 ...
	 * \warning Note that these callbacks are not thread safe when using multithreading, DO NOT call any part of the Unity API from these callbacks except for if(isLogging)Debug.Log
	 */
	...
	public ushort GetNextPathID ()
	{
		if (nextFreePathID == 0) {
			...
			//StartPath (new CleanupPath65K ());
				...
				if(isLogging)Debug.Log ("65K cleanup");
...
#line hidden
	...
	private static void AstarLog (string s) {
		if (System.Object.ReferenceEquals(active,null)) {
			if(isLogging)Debug.Log ("No AstarPath object was found : "+s);
			return;
		}
		
		if (active.logPathResults != PathLog.None && active.logPathResults != PathLog.OnlyErrors) {
			if(isLogging)Debug.Log (s);
		}
	}
		...
		if (threadEnumerator != null) {
			try {
				...
				if (!(e is ThreadControlQueue.QueueTerminationException)) {
					...
					if(isLogging)Debug.LogException (e);
					if(isLogging)Debug.LogError ("Unhandled exception during pathfinding. Terminating.");
	...
	*/
				...
				if (s.order == GraphUpdateOrder.FloodFill) {
					...
					if (threading == GraphUpdateThreading.SeparateAndUnityInit) {
						try {
							...
							if(isLogging)Debug.LogError ("Error while initializing GraphUpdates\n" + e);
						}
					}
					try {
						s.graph.UpdateArea (s.obj);
					} catch (System.Exception e) {
						if(isLogging)Debug.LogError ("Error while updating graphs\n"+e);
					}
				}
			}
		}
	...
	void ProcessGraphUpdatesAsync (System.Object _astar) {
		...
		if (System.Object.ReferenceEquals (astar, null)) {
			if(isLogging)Debug.LogError ("ProcessGraphUpdatesAsync started with invalid parameter _astar (was no AstarPath object)");
			return;
		}
		...
		while (!astar.pathQueue.IsTerminating) {
			...
			while (graphUpdateQueueAsync.Count > 0) {
				...
				try {
					...
					if(isLogging)Debug.LogError ("Exception while updating graphs:\n"+e);
				}
			}
	...
	public static int CalculateThreadCount (ThreadCount count) {
		if (count == ThreadCount.AutomaticLowLoad || count == ThreadCount.AutomaticHighLoad) {
			...
			if ( memory <= 0 ) {
				if(isLogging)Debug.LogError ("Machine reporting that is has <= 0 bytes of RAM. This is definitely not true, assuming 1 GiB");
				memory = 1024;
			}
	...
	public void Awake () {
		...
		if (FindObjectsOfType (typeof(AstarPath)).Length > 1) {
			if(isLogging)Debug.LogError ("You should NOT have more than one AstarPath component in the scene at any time.\n" +
		...
		for (int i=0;i<threads.Length;i++) {
			if (logPathResults == PathLog.Heavy)
				if(isLogging)Debug.Log ("Starting pathfinding thread "+i);
	...
	public void OnDestroy () {
		...
		if (logPathResults == PathLog.Heavy)
			if(isLogging)Debug.Log ("+++ AstarPath Component Destroyed - Cleaning Up Pathfinding Data +++");
		...
		if (logPathResults == PathLog.Heavy)
			if(isLogging)Debug.Log ("Processing Eventual Work Items");
		...
		if (threads != null) {
			for (int i=0;i<threads.Length;i++) {
				...
				if (!threads[i].Join(200)) {
					if(isLogging)Debug.LogError ("Could not terminate pathfinding thread["+i+"] in 200ms." +
						"Not good.\nUnity webplayer does not support Thread.Abort\nHoping that it will be terminated by Unity WebPlayer");
				}
#else
				if (!threads[i].Join (50)) {
					if(isLogging)Debug.LogError ("Could not terminate pathfinding thread["+i+"] in 50ms, trying Thread.Abort");
					threads[i].Abort ();
				}
#endif
			}
		...
		if (logPathResults == PathLog.Heavy)
			if(isLogging)Debug.Log ("Returning Paths");
		...
		if (logPathResults == PathLog.Heavy)
			if(isLogging)Debug.Log ("Destroying Graphs");
		...
		if (logPathResults == PathLog.Heavy)
			if(isLogging)Debug.Log ("Cleaning up variables");
...
#region ScanMethods
	...
	public void FloodFill (GraphNode seed, uint area) {
		...
		if (area > GraphNode.MaxRegionCount) {
			if(isLogging)Debug.LogError ("Too high area index - The maximum area index is " + GraphNode.MaxRegionCount);
			return;
		}
		
		if (area < 0) {
			if(isLogging)Debug.LogError ("Too low area index - The minimum area index is 0");
			return;
		}
	...
	public void FloodFill () {
		...
		if (warnAboutAreas) {
			if(isLogging)Debug.LogError ("Too many areas - The maximum number of areas is " + GraphNode.MaxRegionCount +". Try raising the A* Inspector -> Settings -> Min Area Size value. Enable the optimization ASTAR_MORE_AREAS under the Optimizations tab.");
		}
	...
	*	 if(isLogging)Debug.Log ("Scanning... " + progress.description + " - " + (progress.progress*100).ToString ("0") + "%");
	  * } \endcode
	  * \see Scan
	  */
	...
	public void ApplyLinks () {
		...
		if (astarData.userConnections != null && astarData.userConnections.Length > 0) {
			if(isLogging)Debug.LogWarning ("<b>Deleting all links now</b>, but saving graph data in backup variable.\nCreating replacement links using the new system, stored under the <i>Links</i> GameObject.");
	...
	public static void WaitForPath (Path p) {
		...
		if (waitForPathDepth == 5) {
			if(isLogging)Debug.LogError ("You are calling the WaitForPath function recursively (maybe from a path callback). Please don't do this.");
		}
	...
	/** Will send a callback when it is safe to update nodes. This is defined as between the path searches.
	  ...
	  * DO NOT call any part of the Unity API from those callbacks except for if(isLogging)Debug.Log
	  * 
	  * \code
...
\endcode
	...
	public static void StartPath (Path p, bool pushToFront = false) {
		...
		if (active == null) {
			if(isLogging)Debug.LogError ("There is no AstarPath object in the scene");
			return;
		}
		...
		if (active.graphs == null || active.graphs.Length == 0) {
			if(isLogging)Debug.LogError ("There are no graphs in the scene");
			p.Error ();
			p.LogError ("There are no graphs in the scene");
			if(isLogging)Debug.LogError (p.errorLog);
			return;
		}
	...
	public void OnApplicationQuit () {
		if (logPathResults == PathLog.Heavy) {
			if(isLogging)Debug.Log ("+++ Application Quitting - Cleaning Up +++");
		}
...
#region MainThreads
	...
	void CalculatePathsThreaded (System.Object _threadInfo) {
		...
		try {
			...
			if(isLogging)Debug.LogError ("Arguments to pathfinding threads must be of type ThreadStartInfo\n"+e);
		...
		try {
			...
			if (e is System.Threading.ThreadAbortException || e is ThreadControlQueue.QueueTerminationException) {
				if (astar.logPathResults == PathLog.Heavy)
					if(isLogging)Debug.LogWarning ("Shutting down pathfinding thread #"+threadInfo.threadIndex+" with Thread.Abort call");
				return;
			}
			if(isLogging)Debug.LogException (e);
			if(isLogging)Debug.LogError ("Unhandled exception during pathfinding. Terminating.");
		...
		if(isLogging)Debug.LogError ("Error : This part should never be reached.");
